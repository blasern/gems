...
hazardf[[1,3]] <- function(t, shape, scale, bl, history) {shape[bl+1]/scale*(t/scale)^(shape[bl+1]-1)}
# transition 3: 2->3:  history-dependent Weibull
hazardf[[2,3]] <- function(t, shape, scale, history) {
  shape/sum(history)/scale*(t/scale)^(shape/sum(history)-1)}
