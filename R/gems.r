...
setClass("ArtCohort",
         representation(
           ...
           baseline = "matrix",
           follow.up = "numeric",
           parameterss= "transition.structure",
...
setMethod( "update", "ArtCohort", function(object, newsize, addbaseline=matrix(NA, nrow = newsize - object@size), newInitialStates=rep(1, newsize - object@size)){ 
  aux1 =
    simulateCohort(
      ...
      parameterss = object@parameterss,
    ...
    if (length(hazardf[[tr]]) > 0) {
      if (class(hazardf[[tr]]) != "function") {
        if (hazardf[[tr]] != "Weibull" && hazardf[[tr]] != 
            "multWeibull" && hazardf[[tr]] != "impossible" && 
            !is.na(hazardf[[tr]])) 
...
simulateCohort <-
  function(transitionFunctions, 
    ...
    parameters =  parameterss@list.matrix
    ...
    sigma <- matrix(0, ncol=length(unlist(parameters)), nrow=length(unlist(parameters)))
    ...
    simulatedcohort@parameterss <- parameterss
